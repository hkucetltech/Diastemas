...
Date.abbrDayNames=["Sun","Mon","Tues","Wed","Thur","Fri","Sat"];
Date.monthNames=["Jan.","Feb.","Mar.","Apr.","May.","Jun.","Jul.","Aug.","Sept.","Oct.","Nov.","Dec."];
Date.abbrMonthNames=["Jan.","Feb.","Mar.","Apr.","May.","Jun.","Jul.","Aug.","Sept.","Oct.","Nov.","Dec."];
...
function()
{
	...
	b("addMonths",function(d){var c=this.getDate();this.setMonth(this.getMonth()+d);if(c>this.getDate()){this.addDays(-this.getDate())}return this});
	...
	b("zeroTime",function(){this.setMilliseconds(0);this.setSeconds(0);this.setMinutes(0);this.setHours(0);return this});
		...
		for(var h=0;h<e.length;h++)
		{
			...
			switch(e[h])
			{
				...
				case"M":
					...
					{
						if(Date.abbrMonthNames[g].toLowerCase()==o){break}
					}
					p.setMonth(g);
					break;
				case"y":
...
function(d)
{
	d.fn.extend(
				{
					...
					{
						var C=function(i){return document.createElement(i)};
						...
						if(t.showHeader!=d.dpConst.SHOW_HEADER_NONE)
						{
							...
							{
								var h=x%7;var v=Date.dayNames[h];
								o.append(jQuery(C("th")).attr({scope:"col",abbr:v,title:v,"class":(h==0||h==6?"weekend":"weekday")})
								/*星期字数*/
								.html(t.showHeader==d.dpConst.SHOW_HEADER_SHORT?v.substr(0,3):v))
							}
						}
						var e=d(C("table")).attr({cellspacing:0,cellpadding:0}).addClass("jCalendar").append((t.showHeader!=d.dpConst.SHOW_HEADER_NONE?d(C("thead")).append(o):C("thead")));
						var f=d(C("tbody"));
						var z=(new Date()).zeroTime();
						var B=t.month==undefined?z.getMonth():t.month;
						var p=t.year||z.getFullYear();var m=new Date(p,B,1);
						...
						var A=function(i)
						{
							return function()
							{
								if(t.hoverClass)
								{
									...
									{
										if(i&&!r.is(".disabled")){r.parent().addClass("activeWeekHover")}
									}
								}
							}
						};
						...
						while(n++<q)
						{
							...
							{
								var j=m.getMonth()==B;
								var y=d(C("td")).text(m.getDate()+"").addClass((j?"current-month ":"other-month ")+(m.isWeekend()?"weekend ":"weekday ")+(j&&m.getTime()==z.getTime()?"today ":"")).data("datePickerDate",m.asString()).hover(A(k),g);u.append(y);
					...
					{
						if(!d.event._dpCache){d.event._dpCache=[]}
						...
						return this.each(function()
												  {
													  ...
													  if(!i&&e.createButton)
													  {
														  f.button=d('<a href="#" class="dp-choose-date" title="'+d.dpText.TEXT_CHOOSE_DATE+'">'+d.dpText.TEXT_CHOOSE_DATE+"</a>").bind("click",function(){g.dpDisplay(this);this.blur();return false});
														  g.after(f.button)
													  }
													  if(!i&&g.is(":text"))
													  {
														  g.bind("dateSelected",function(k,j,l){this.value=j.asString()}).bind("change",function()
																																				 {
																																					 if(this.value=="")
																																					 {
																																						 f.clearSelected()
	...
	d.extend(
			 a.prototype,
			 {
				...
				{
					if(e){this.startDate=Date.fromString(e)}
					if(!this.startDate)
					{
						var tmpDate=new Date();
						tmpDate.setDate(tmpDate.getDate() - 3650);
//						if(tmpDate.getHours()<=17)
//						{
...
//						}
						this.startDate=tmpDate.zeroTime();
					}
					this.setDisplayedMonth(this.displayedMonth,this.displayedYear)
				},
				setEndDate:function(e)
				{
					if(e){this.endDate=Date.fromString(e)}
					if(!this.endDate)
					{
						var tmpDate=new Date();
						tmpDate.setDate(tmpDate.getDate() + 3650);
						this.endDate=tmpDate.zeroTime();
					}
				...
				{
					if(l<this.startDate||l>this.endDate){return}
					var g=this.settings;
					if(g.selectWeek)
					{
					...
					{
						if(e&&this.numSelected==this.numSelectable){return}
					}
				...
				display:function(e)//顯示日曆
				{
					if(d(this.ele).is(".dp-disabled")){return}
					e=e||this.ele;
					var m=this;
					var i=d(e);
					var l=i.offset();
					...
					if(m.inline)
					{
						n=d(this.ele);
						o={
							id:"calendar-"+this.ele._dpId,"class":"dp-popup dp-popup-inline"
						};
						d(".dp-popup",n).remove();
						j={}
					}
					else
					{
						n=d("body");
						o={
							id:"dp-popup","class":"dp-popup"
						};
						j={
							top:l.top+m.verticalOffset+20,left:l.left+m.horizontalOffset
						};
						var k=function(r)
						{
							var p=r.target;
						...
						d(document).bind("keydown.datepicker",function(p)
																	   {
																		   if(p.keyCode==27){m._closeCalendar()}
																	   })
				   }
				   ...
				   n.append(
							d("<div></div>").attr(o).css(j).append(d("<h2></h2>"),
																   d('<div class="dp-nav-prev"></div>').append(
																											   d('<a id="dp-nav-prev-year" href="#" title="'+d.dpText.TEXT_PREV_YEAR+'">&laquo;</a>').bind("click",function(){return m._displayNewMonth.call(m,this,0,-1)}),
																											   d('<a id="dp-nav-prev-month" href="#" title="'+d.dpText.TEXT_PREV_MONTH+'">&lt;</a>').bind("click",function(){return m._displayNewMonth.call(m,this,-1,0)})
																											  ),
																   d('<div class="dp-nav-next"></div>').append(
																											   d('<a id="dp-nav-next-year" href="#" title="'+d.dpText.TEXT_NEXT_YEAR+'">&raquo;</a>').bind("click",function(){return m._displayNewMonth.call(m,this,0,1)}),
																											   d('<a id="dp-nav-next-month" href="#" title="'+d.dpText.TEXT_NEXT_MONTH+'">&gt;</a>').bind("click",function(){return m._displayNewMonth.call(m,this,1,0)})
																											  ),
				   ...
				   {
					   f.append(d('<a href="#" id="dp-close">'+d.dpText.TEXT_CLOSE+"</a>").bind("click",function(){m._closeCalendar();return false}))
				   }
				   m._renderCalendar();
				...
				cellRender:function(i,e,g,f)
				{
					...
					{
						if(j.selectMultiple&&j.numSelected==j.numSelectable){i.addClass("unselectable")}
					}
				},
				...
				_renderCalendar:function()
				{
					...
					d(".dp-calendar",this.context).renderCalendar(d.extend(
																		   {},
																		   this.settings,
																		   {month:this.displayedMonth,year:this.displayedYear,renderCallback:this.cellRender,dpController:this,hoverClass:this.hoverClass}
																		   )
																  );
	...
	d.dpText={TEXT_PREV_YEAR:"Prev year",TEXT_PREV_MONTH:"Prev month",TEXT_NEXT_YEAR:"Next year",TEXT_NEXT_MONTH:"Next year",TEXT_CLOSE:"close",TEXT_CHOOSE_DATE:"",HEADER_FORMAT:"mmmm yyyy"};
	...
	{
		d.fn.bgIframe=function(){return this}
	}
